...
keywords = [ "ragazzo", "bambino", "tuta", "pantaloni", "scia", "bambini", "ragazzi" ]

max = 1 #max number of pages to parse
anno = 2007
 
url = "https://www.subito.it/annunci-italia/vendita/usato/?q=sci" #url to parse

pageNumb = 1 #current page number
suffix = "&o=" #suffix to add to the url            

listItems = "{ \'list\': [" #list of items
totFound = 0 #total number of items found
...
while pageNumb <= max: #max is the number of pages to scrape
    try:
        ...
        for t in data["props"]["state"]["items"]["list"]: #for each item in the page
            ...
            if not containsKeyWords(t["item"]["subject"] + " " + t["item"]["body"], outputStr) and int(t["item"]["features"]["/price"]["values"][0]["key"]) <= 100:
...
open("speroScraping\prova.json", "w").write(listItems.lower()) #write the list of items to a json file

print("Total items found: " + str(totFound))